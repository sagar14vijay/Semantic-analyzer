...
assignment_statement
    :types ID ASSIGNMENT char_expression {if (!strcmp($1, "char")){printf("%s = %c ;\n", $2 -> name, $4 );$2 -> type = "char";flag = $2->name[0]-'a';declared[flag]=1;} else{yyerror("type missmatch ; char assignment to int expected 'char' ");} 	removetemp(tempnum);  }  
    | ID ASSIGNMENT char_expression {if(declared[$1->name[0]-'a']==1){if( !strcmp($1 -> type, "char")){printf("%s = %c ;\n", $1 -> name, $3 );}else{yyerror("type miss match ");} removetemp(tempnum);}else {yyerror("undeclared variable used");} }

	| types ID ASSIGNMENT expression { value = strtod($4,&ptr);if(value>2147483647){printf("********warning for overflow****** \n");}if (!strcmp($1, "int")){printf("%s = %s ;\n", $2 -> name, $4 ); $2 -> type = "int";flag = $2->name[0]-'a';declared[flag]=1;} else{yyerror("type missmatch ; int assignment to char ; expected 'int' "); removetemp(tempnum);}}
    | ID ASSIGNMENT expression {if(declared[$1->name[0]-'a']==1){if( !strcmp($1 -> type, "int")){printf("%s = %s ;\n", $1 -> name, $3 );}else{yyerror("type miss match ");} removetemp(tempnum);}else {yyerror("undeclared variable used");}}
...
expression
    : NUMBER { $$ = $1; }
    | ID { if(declared[$1->name[0]-'a']==1){$$ = $1 -> name;}else {yyerror("undeclared variable in expression");}}
